...
python -m grpc_tools.protoc \
    -I ./src/backend_services/{service}/rpc" \
    --pyi_out="./src/backend_services/{service}/proto" \
    --python_out="./src/backend_service/{service}/proto" \
    --grpc_python_out="./src/backend_services/{service}/proto" \
    "./src/backend_services/{service}/rpc/{your_protofile_here.proto}"
```

This command generates two python files that can be used to communicate between the services.
To use the command, replace both `{service}` and `{your_protofile_here.proto}`
with the name of the service and protofile you want to generate then run in the terminal.
...
```
./src/backend_services/utils/rpc/generate_protofiles.ps1
```

This command will execute the python protofile generation command for every protofile within `src/backend_services/{service}/rpc` file location.

Once ran, the execution policy will be reset to its default settings.
